...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        ##### this is too large!!!
        final_attention_probs = torch.zeros(hidden_states.size(0), self.num_attention_heads, hidden_states.size(1), hidden_states.size(1), device=hidden_states.device) 
        ...
        for mask_index, head_mask in enumerate([head_mask1, head_mask2]):
            ...
            final_attention_probs = final_attention_probs + attention_probs * head_mask

            context_layer = torch.matmul(final_attention_probs, value_layer)
        ...
        outputs = (final_context_layer, final_attention_probs) if self.output_attentions else (final_context_layer,)
