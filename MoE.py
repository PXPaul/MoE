...
class MaskedLinear(nn.Linear):
    def __init__(
        ...
        self.gate_lin = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(in_features, self.num_masks)
        )
    ...
    def forward(self, input: torch.tensor, threshold: float):
        ...
        for mask_index, mask in enumerate([mask1, mask2]):
           ...
           element_mask = selected_mask_index == mask_index # [3, 5, 4] 
           # Select which inputs are relevant to the mask we are using this iteration
           relevant_inputs = torch.where(element_mask, input, 0) # [3, 5, 4] 
           # Masked weight for iteration
           masked_weight = mask * self.weight # [6,4]
           # We only write the relevant rows at each iteration so ok to just add
           output += relevant_inputs @ masked_weight.T # [3,5,6]
